...
void Stage_class::getThorStages(OutThor* Out_Thor) { // get serial numbers list for LTS devices that are connected
    char desc[65];
...
string Stage_class::selectThor(vector<char*> serialNos, int Thor_sel) { // Selec which thor labs stage to connect to
    string serial;
    serial = *serialNos[Thor_sel];
    return serial;
};

void Stage_class::getStage(myData* Data) { // set stage name

    Data->n_stages = 1;
    int Inits;
    OutThor Out_Thor; // a struct containing serial numbers list for LTS devices that are connected
    stageObject_M = new CTango;
    stageObject_P = new PriorScientificSDK;
    ...
    case 'Prio':
        ...
        Inits = PriorScientificSDK_Initialise(); // initialize dll api, to configure internal data structures
...
void Stage_class::connectStage(myData* Data) { // connects stage, returns serial, id, stage object, error, index
    //stageObject_M = new CTango();
    char buf[50];
    ...
    case 'Thor':
        ...
        try {
            ...
            sprintf(buf, "%s, %s is successfully connected", testSerialNo, Data->model);
            Data->msg = *buf;
        }
        catch (...) {
...
void Stage_class::goHome(myData* Data) { // move stage home
    char buf[50];
    switch (Data->stage_name) {
    case 'Thor':
        char testSerialNo[16];
...
void Stage_class::getAllParams(myData* Data) { // get all parameters (step, position, velocity, acceleration)
...
void Stage_class::disconnectStage(myData* Data) { // disconnect stage
    char buf[50];
    switch (Data->stage_name) {
    case 'Thor':
        char testSerialNo[16];
...
void Stage_class::stopMotion(myData* Data) { // suspend all stage movements
...
void Stage_class::setAllParams(myData* Data) { // set all parameters (step, position, velocity, acceleration)
...
void Stage_class::getStep(myData* Data) { // get stage step size
    char buf[50];
    switch (Data->stage_name) {
    case 'Thor':
        char testSerialNo[16];
...
void Stage_class::setStep(myData* Data) { // set stage step size
...
void Stage_class::getVelParams(myData* Data) { // get velocity parameters (vel and acc)
...
void Stage_class::setVelParams(myData* Data) { // get velocity parameters
    switch (Data->stage_name) {
...
void Stage_class::moveAbs(myData* Data) { // move stage to Absolute position
    switch (Data->stage_name) {
    case 'Thor':
        char testSerialNo[16];
        strcpy(testSerialNo, Data->serial.c_str());
...
void Stage_class::getPos(myData* Data) { // get current stage position
    int i;
    char* chars_array;
...
void Stage_class::isBusy(myData* Data) { // is stage moving
    int choice;
...
void Stage_class::getLastError(myData* Data) { // get last error
    switch (Data->stage_name) {
    case 'Thor':
...
void Stage_class::enableJoystick(myData* Data, int state) { //enable / disable joystick
